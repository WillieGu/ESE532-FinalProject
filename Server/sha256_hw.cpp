...
void sha256_update_hw(SHA256_CTX_HW *ctx, const BYTE data[], size_t len) {
    for (size_t i = 0; i < len; ++i) {
        ...
        ctx->datalen++;
        if (ctx->datalen == 64) {
            sha256_transform_hw(ctx, ctx->data);
            ctx->bitlen += 512;
            ctx->datalen = 0;
        }
    }
...
void sha256_hash_hw(const BYTE data[], size_t length, BYTE hash_output[]) {
    SHA256_CTX_HW ctx;
    sha256_init_hw(&ctx);
    sha256_update_hw(&ctx, data, length);
    sha256_final_hw(&ctx, hash_output);
}
...
void sha256_hash_range_hw(const BYTE data[], uint32_t start_idx, uint32_t end_idx, BYTE hash_output[]) {
    SHA256_CTX_HW ctx;
    sha256_init_hw(&ctx);
    sha256_update_hw(&ctx, data + start_idx, end_idx - start_idx);
    sha256_final_hw(&ctx, hash_output);
}
...
std::string sha256_hash_string_hw(const BYTE data[], size_t length) {
    ...
    sha256_hash_hw(data, length, hash);
    return bytes_to_hex_string_hw(hash, SHA256_BLOCK_SIZE);
}
