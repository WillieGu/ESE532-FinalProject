...
uint16_t dictionary[MAX_DICT_SIZE][2] = {0};  // Global dictionary

void initialize_dictionary() {
    static bool initialized = false;  // Static flag to ensure one-time initialization
    if (!initialized) {
        for (uint16_t i = 0; i < 256; ++i) {
            dictionary[i][0] = 0xFFFF;  // No prefix for single characters
            dictionary[i][1] = i;       // Character itself
        }
        initialized = true;  // Set flag to true after initialization
    }
}

void lzw_encode(const unsigned char *input_data, uint16_t input_size, uint8_t compressed_data[OUTPUT_SIZE],
    ...
    for (uint16_t i = 1; i < input_size; ++i) {
        ...
        for (uint16_t j = 256; j < dict_size; ++j) {
            if (dictionary[j][0] == current_code && dictionary[j][1] == next_char) {
                current_code = j;
                found = true;
                break;
            }
        ...
        if (!found) {
            ...
            if (dict_size >= MAX_DICT_SIZE - 1) {
                std::cerr << "Debug: Dictionary is near full. Current size: " << dict_size << std::endl;
            }
            // Add new string to the dictionary if there's space
            if (dict_size < MAX_DICT_SIZE) {
                dictionary[dict_size][0] = current_code;
                dictionary[dict_size][1] = next_char;
                ++dict_size;
            }
    ...
    if (compressed_size == 0) {
        std::cerr << "Error: Compressed size is zero. This should not happen for valid input.\n";
        *compressed_size_buf = 0;
        return;
    }
